...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _initServices() async {
    ...
    if (mcpPath == null || mcpPath.isEmpty) {
      (mcpPath, mcpArgs) = await _findMCPPath();
    }
    ...
    try {
      ...
      await _mcpClient!.start(args: mcpArgs, environment: mcpEnv);
  ...
  Future<(String, List<String>)> _findMCPPath() async {
    // 1. Check bundled resources (macOS mostly)
    // Platform.resolvedExecutable points to .../Contents/MacOS/AGNTCY Directory
    final exeDir = File(Platform.resolvedExecutable).parent;
    final resourcesDir = '${exeDir.parent.path}/Resources';

    var mcpPath = '$resourcesDir/mcp-server';
    debugPrint('Checking Bundle Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      return (mcpPath, <String>[]);
    }

    mcpPath = '$resourcesDir/dirctl';
    debugPrint('Checking Bundle Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      return (mcpPath, ["mcp", "serve"]);
    }

    // 2. Check same directory (Linux/Windows bundled)
    mcpPath = Platform.isWindows ? '${exeDir.path}/mcp-server.exe' : '${exeDir.path}/mcp-server';
    debugPrint('Checking Local Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      return (mcpPath, <String>[]);
    }

    mcpPath = Platform.isWindows ? '${exeDir.path}/dirctl.exe' : '${exeDir.path}/dirctl';
    debugPrint('Checking Local Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      return (mcpPath, ["mcp", "serve"]);
    }
    ...
    mcpPath = Platform.isWindows ? '$binDir/mcp-server.exe' : '$binDir/mcp-server';
    debugPrint('Checking Dev Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      // Normalize path for clean printing
      return (File(mcpPath).absolute.path, <String>[]);
    }

    mcpPath = '$binDir/dirctl';
    debugPrint('Checking Dev Path: $mcpPath');
    if (await File(mcpPath).exists()) {
      return (File(mcpPath).absolute.path, ["mcp", "serve"]);
    }
